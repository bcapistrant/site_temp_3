...
This post introduces some of the workshop's assets --- namely, 1. a [framework](data_decisiontree.png){:target="_blank"} to identify relevant data sources and 2. a [catalog](https://docs.google.com/spreadsheets/d/1GV2QTW7nDaPKmCzPvRGCYKrTY1jI1aFDt1-I9lBnfFY/edit?usp=sharing){:target="_blank"} of some of those datasets' features --- and explains how they might be useful to someone wanting to get a quick answer with relatively low lift.
...
**I made [a catolog / table](https://docs.google.com/spreadsheets/d/1GV2QTW7nDaPKmCzPvRGCYKrTY1jI1aFDt1-I9lBnfFY/edit?usp=sharing){:target="_blank"} with all of this information in the diagram below for each study.** If you know already what data you need, or want to choose based on one of the factors below (e.g., sample size), this would be a good place to start.
...
And with this specific user -- again, someone in product, ops, or strategy who needs a quick answer without the hassle of raw data -- in mind, _is there an online analysis tool?_ Which places can you get a good (enough) answer (relatively) quickly and easily? Many, though not all, surveys have some such tool.

*The surveys with visualization tools and query functions are:*

- Behavioral Risk Factor Surveillance System ([BRFSS](https://www.cdc.gov/brfss/data_tools.htm){:target="_blank"})
- National Health and Nutrition Examination Survey ([NHANES](https://www.cdc.gov/nchs/nhanes/visualization/){:target="_blank"})
- Medical Expenditure Panel Survey (MEPS): [Household](https://datatools.ahrq.gov/meps-hc){:target="_blank"} or [Individual](https://datatools.ahrq.gov/meps-ic){:target="_blank"} Data
- Consumer Assessment of Healthcare Providers and Systems ([CAHPS](https://datatools.ahrq.gov/cahps){:target="_blank"})
- Healthcare Costs & Utilization Project ([HCUP](https://datatools.ahrq.gov/hcupnet-dua){:target="_blank"})
- National Study on Drug Use and Health ([NSDUH](https://pdas.samhsa.gov/#/){:target="_blank"})
- National Health Interview Survey ([NHIS](https://www.cdc.gov/nchs/nhis/shs.htm){:target="_blank"})
- [National Survey of Childrens' Health](https://www.childhealthdata.org/){:target="_blank"}
- Yourh Risk Behavior Survey System ([YRBSS](https://nccd.cdc.gov/Youthonline/App/Default.aspx){:target="_blank"})

### What can you get from these online analysis tools?
As a glimpse of what some of these analysis tools look like and some kinds of common features, BRFSS is a good example. It has the largest sample size and the best capacity for a state or metro-area analysis of the other studies. The [Prevalence and Trends Data Tool](https://www.cdc.gov/brfss/brfssprevalence/index.html){:target="_blank"} allows users to select one of the survey questions and see the state average responses displayed in a map, graph, or table. There are some additional features for more detailed analyses:
...
-   Not all online analysis tools are very intuitive to use. For example, [this more sophisticated analysis tool with BRFSS](https://chronicdata.cdc.gov/d/dttw-5yxu/visualization){:target="_blank"} data gives substantially more features and flexibility --- if you know what you're trying to make. I suspect users who don't spend a lot of time in data dashboarding GUIs may find this a challenging way to get to a final result.
...
-   *Other government agency data:* Ancillary data like Census, Labor department data on workforce issues, or more direct health related data from other agencies (ex: NHTSA has an EMS data source, [NEMSIS](https://nemsis.org/){:target="_blank"}) may be useful for specific use cases, maybe a version 2.0
...
-   Federal data search engines and data aggregators
    -   [Healthcare.gov](https://data.healthcare.gov/datasets){:target="_blank"}
    -   [Healthdata.gov](https://healthdata.gov/){:target="_blank"}
    -   [CDC Wonder](https://wonder.cdc.gov/){:target="_blank"}
-   Public data searches, that likely include data on specific topics, that may be less generalizable, and more likely requires working with the raw data:
    ...
    -   [Google Dataset Search](http://dataset.research.google.com/){:target="_blank"}
-   Academic hosts, non-profits, and other aggregators of data:
    -   [IPUMS](https://www.ipums.org/){:target="_blank"} (demographic, economic, and health data --- over time)
    ...
    -   [ICPSR](https://www.icpsr.umich.edu/){:target="_blank"} (Lots of NIH funded studies' data ishosted here)
    -   [NEMSIS](https://rp.nemsis.org/reportportal/login.aspx){:target="_blank"}
...
-   Creating new tools: analysts who do work with the original raw data could prioritize extending existing tools with interactive dashboard tools like [Shiny](https://shiny.rstudio.com/){:target="_blank"} or [Plotly](https://plotly.com/){:target="_blank"}

## Wrapping it up

There is lots of data, generally from federally funded surveys, that can serve common business needs in healthcare and health tech. Two use cases -- benchmarking your prevalence against an average, and identifying the market size for a new product -- are particularly well suited to quick answers that many common data sources have. *The [decision tree](data_decisiontree.png){:target="_blank"} and [catalog/table](https://docs.google.com/spreadsheets/d/1GV2QTW7nDaPKmCzPvRGCYKrTY1jI1aFDt1-I9lBnfFY/edit?usp=sharing){:target="_blank"} of surveys and their contents hopefully help find a useful solution more quickly.* Most surveys have an online analysis tools that will do the job for basic uses, and it's worth getting to know one or two of these tools to get these sort of estimates fast and easily. They aren't perfect, but they're definitely better than nothing -- and can avoid bugging your data team or dusting off old intro stats programming knowledge to get there yourself with the raw data.
